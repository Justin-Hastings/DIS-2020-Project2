...
function displayclasses() {
	...
	if(xhr.readyState==0 || xhr.readyState==4){
		...
		xhr.onreadystatechange=displayclassesresponse;
		xhr.send(null);
	}
}

function displayclassesresponse() {
	...
	if(xhr.readyState==4 && xhr.status==200){
		...
		classes.forEach(function(item) {
			classesRows += '<tr><td>' + item["class"] + '</td><td>' + item.handicap + '</td></tr>';
		} );
		
		classesRows = '<table id="bclasses"><tr><th>Class</th><th>handicap</th></tr>' + classesRows + '</table>';
		document.getElementById('classresults').innerHTML = classesRows;
